...
public class GoodsDao {
	// 상품 수정
	public ArrayList<HashMap<String, Object>> modifyGoods(Connection conn, int goodsCode) throws Exception {
		ArrayList<HashMap<String, Object>> list = new ArrayList<HashMap<String,Object>>();
		String sql = "UPDATE
		PreparedStatement stmt = conn.prepareStatement(sql);
		stmt.setInt(1, goodsCode);
		System.out.println("Dao굿즈코드->"+goodsCode);
	...
	public ArrayList<HashMap<String, Object>> selectGoodsOne(Connection conn, int goodsCode) throws Exception {
		ArrayList<HashMap<String, Object>> list = new ArrayList<HashMap<String,Object>>();
		String sql = "SELECT gs.goods_code goodsCode, gs.goods_name goodsName, gs.goods_price goodsPrice, gs.goods_memo goodsMemo, img.filename filename, img.content_type contentType"
				+ 	" FROM goods gs INNER JOIN goods_img img"
				+ 	" ON gs.goods_code = img.goods_code"
				+ 	" WHERE gs.goods_code = ?";
		PreparedStatement stmt = conn.prepareStatement(sql);
	...
	*/
	...
	public HashMap<String, Integer> insertItem(Connection conn, Goods goods) throws Exception {
		String sql = "INSERT INTO goods(goods_name, goods_price, goods_memo, soldout, emp_id, createdate) VALUES(?,?,?,?,?,NOW())";
		...
		System.out.println(goods.getGoodsMemo()+"<-상품설명");
